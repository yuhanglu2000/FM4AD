...
class VAD_vlm_fusion(VAD):
    def forward_pts_train(self,
                          ...
                          neg_traj=None):
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev,
                                  ego_his_trajs=ego_his_trajs, ego_lcf_feat=ego_lcf_feat, pos_traj=pos_traj, neg_traj=neg_traj)
        loss_inputs = [
            gt_bboxes_3d, gt_labels_3d, map_gt_bboxes_3d, map_gt_labels_3d,
            outs, ego_fut_trajs, ego_fut_masks, ego_fut_cmd, gt_attr_labels
        ]
    ...
    def forward_train(self,
        ...
        losses_pts = self.forward_pts_train(img_feats, gt_bboxes_3d, gt_labels_3d,
                                            ...
                                            ego_fut_masks=ego_fut_masks, ego_fut_cmd=ego_fut_cmd,
                                            ego_lcf_feat=ego_lcf_feat, gt_attr_labels=gt_attr_labels,
                                            pos_traj=pos_traj, neg_traj=neg_traj)
...
def format_white_box_results(gt_bboxes_3d, gt_attr_labels, map_gt_bboxes_3d, fut_ts=6):
    outs = {}
    if isinstance(gt_bboxes_3d, list):
        assert len(gt_bboxes_3d) == 1
        gt_bboxes_list = [torch.cat(
            (gt_bboxes.gravity_center, gt_bboxes.tensor[:, 3:]),
            dim=1) for gt_bboxes in gt_bboxes_3d]
        gt_bboxes_info = gt_bboxes_list[0]
        gt_bboxes_info = gt_bboxes_info.cuda()
    if isinstance(gt_attr_labels, list):
        assert len(gt_attr_labels) == 1
        gt_attr_label = gt_attr_labels[0]
        gt_fut_trajs = gt_attr_label[:, :fut_ts*2]
        gt_fut_masks = gt_attr_label[:, fut_ts*2:fut_ts*3]
        gt_fut_trajs = gt_fut_trajs.cuda()
        gt_fut_masks = gt_fut_masks.cuda()
    assert gt_fut_trajs.shape[0] == gt_bboxes_info.shape[0]
    bbox_info = torch.cat((gt_bboxes_info, gt_fut_trajs), dim=-1)
    outs['agent_info'] = bbox_info
    
    if isinstance(map_gt_bboxes_3d, list):
        assert len(map_gt_bboxes_3d) == 1
        map_gt_vecs_list = copy.deepcopy(map_gt_bboxes_3d)
        map_gt_bboxes_list = [
            map_gt_bboxes.bbox for map_gt_bboxes in map_gt_vecs_list]
        map_gt_pts_list = [
            map_gt_bboxes.fixed_num_sampled_points for map_gt_bboxes in map_gt_vecs_list]
        map_gt_bboxes = map_gt_bboxes_list[0]
        map_gt_pts = map_gt_pts_list[0].reshape(-1, 20*2)
        map_gt_pts = map_gt_pts.cuda()
        map_gt_bboxes = map_gt_bboxes.cuda()
    map_info = torch.cat((map_gt_pts, map_gt_bboxes), dim=-1)
    outs['map_info'] = map_info
    return outs
    
@DETECTORS.register_module()
class VAD_white_box_fusion(VAD):
    def forward_pts_train(self,
                          pts_feats,
                          gt_bboxes_3d,
                          gt_labels_3d,
        ...
        white_box_info = format_white_box_results(gt_bboxes_3d, gt_attr_labels, map_gt_bboxes_3d)
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev,
                                  ego_his_trajs=ego_his_trajs, ego_lcf_feat=ego_lcf_feat, white_box_info=white_box_info)
        loss_inputs = [
            gt_bboxes_3d, gt_labels_3d, map_gt_bboxes_3d, map_gt_labels_3d,
            outs, ego_fut_trajs, ego_fut_masks, ego_fut_cmd, gt_attr_labels
        ]
