...
class VADFusionHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(VADFusionHead, self).__init__(*args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.map_code_weights = nn.Parameter(torch.tensor(
            self.map_code_weights, requires_grad=False), requires_grad=False)
    ...
    def _init_layers(self):
        ...
        self.pos_traj_encoder = nn.Sequential(
            nn.Linear(12, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims)
        )
        self.neg_traj_encoder = nn.Sequential(
            nn.Linear(12, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims * 2)
        )
        self.fusion_linear = nn.Linear(self.embed_dims * 2, self.embed_dims)
    ...
    def forward(self,
        ...
        assert batch == 1
        if isinstance(pos_traj, list):
            pos_traj = pos_traj[0].to(torch.float32)
        else:
            pos_traj = pos_traj.to(torch.float32)
        if isinstance(neg_traj, list):
            neg_traj = neg_traj[0].to(torch.float32)
        else:
            neg_traj = neg_traj.to(torch.float32)
        
        num_pos, _,_ = pos_traj.shape
        _, num_neg, _,_ = neg_traj.shape
        pos_traj_embed = self.pos_traj_encoder(pos_traj.reshape(1, num_pos, 12)) # bs, 1, dim
        neg_traj_embed = self.neg_traj_encoder(neg_traj.reshape(1, num_neg, 12)) # bs, num_neg_traj, dim
        
        # Interaction
        ego_query = ego_his_feats
        ego_query = self.fusion_linear(torch.cat((ego_query, pos_traj_embed), dim=-1))
        ...
        outs = {
            'bev_embed': bev_embed,
            'ego_query_embed':ego_feats,
            'neg_traj_embed':neg_traj_embed,
            'all_cls_scores': outputs_classes,
            'all_bbox_preds': outputs_coords,
            'all_traj_preds': outputs_trajs.repeat(outputs_coords.shape[0], 1, 1, 1, 1),
            'all_traj_cls_scores': outputs_trajs_classes.repeat(outputs_coords.shape[0], 1, 1, 1),
    ...
    def loss(self,
        ...
        map_enc_pts_preds = preds_dicts['map_enc_pts_preds']
        ego_fut_preds = preds_dicts['ego_fut_preds']
        ego_query_embed = preds_dicts['ego_query_embed']
        neg_traj_embed = preds_dicts['neg_traj_embed']
        ...
        neg_traj_loss_fn = ContrastiveLoss(margin=1.0)
        labels = torch.ones(neg_traj_embed.size(1)).float()
        neg_traj_loss = 0.0
        for i in range(neg_traj_embed.size(1)):
            neg_traj_loss += neg_traj_loss_fn(ego_query_embed.squeeze(0), neg_traj_embed[:, i, :], labels[i])
        neg_traj_loss /= neg_traj_embed.size(1)
        loss_dict['loss_neg_traj'] = neg_traj_loss * 0.001
...
@HEADS.register_module()
class VADWhiteBoxFusionHead(DETRHead):
    def __init__(self,
                 *args,
                 with_box_refine=False,
                 as_two_stage=False,
        ...
        super(VADWhiteBoxFusionHead, self).__init__(*args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)
        self.map_code_weights = nn.Parameter(torch.tensor(
            self.map_code_weights, requires_grad=False), requires_grad=False)
    ...
    def _init_layers(self):
        ...
        self.agent_info_encoder = nn.Sequential(
            nn.Linear(21, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims)
        )
        self.map_info_encoder = nn.Sequential(
            nn.Linear(44, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(),
            nn.Linear(self.embed_dims, self.embed_dims)
        )
        
        self.agent_info_interact = CrossAttentionStack(self.embed_dims, 8, 2)
        self.map_info_interact = CrossAttentionStack(self.embed_dims, 8, 2)
    ...
    def forward(self,
        ...
        agent_res = white_box_info['agent_info']
        map_res = white_box_info['map_info']
        
        agent_res_embed = self.agent_info_encoder(agent_res)
        map_res_embed = self.map_info_encoder(map_res)
        ...
        # ego <-> agent interaction
        agent_res_embed = agent_res_embed.unsqueeze(0)
        agent_query = self.agent_info_interact(agent_query, agent_res_embed, agent_res_embed)
        ...
        map_res_embed = map_res_embed.unsqueeze(0)
        map_query = self.map_info_interact(map_query, map_res_embed, map_res_embed)
...
def move_to_cpu_and_detach(item):
    if isinstance(item, torch.Tensor):
        return item.detach().cpu()
    elif isinstance(item, list):
        return [move_to_cpu_and_detach(sub_item) for sub_item in item]
    elif isinstance(item, dict):
        return {key: move_to_cpu_and_detach(value) for key, value in item.items()}
    else:
        return item